...
class CGMailer extends CApplicationComponent
{
	...
	*/
	public function send($body,$subject='Notificacion Starbox',$priority=1)
	{
		...
		if($mail->Send())
		{
			Yii::log("gemail:[ASUNTO:{$subject}] Se envio a\n\n destinatarios:".$this->getCorreos($this->_dest)."\n\n copias:".$this->getCorreos($this->_destCC)."\n\n ocultas:".$this->getCorreos($this->_destBCC),"trace","email");
			$return=true;
		}else{
			Yii::log("gemail:[ASUNTO:{$subject}] NO se envio a\n\n destinatarios:".$this->getCorreos($this->_dest)."\n\n copias:".$this->getCorreos($this->_destCC)."\n\n ocultas:".$this->getCorreos($this->_destBCC),"error","email");
			$return=false;
		}
		$this->clearAllRecipients();
		return $return;
	...
	protected function getMail()
	{
		...
		// {
			...
			if($this->SMTPAuth){
				...
				$this->_mail->SMTPSecure = $this->SMTPSecure;            			// GMAIL password
			}
